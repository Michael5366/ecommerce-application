    ...
    actions.push({
      ...
      address: {
        ...
        country: address.country,
            ...
            if (JSON.stringify(current.billingAddressIds) !== JSON.stringify(billingSelected) && billingSelected.length > 0) {
              actions.push({ action: 'setDefaultBillingAddress' as const, addressId: billingSelected[0] });
            }
            if (JSON.stringify(current.shippingAddressIds) !== JSON.stringify(shippingSelected) && shippingSelected.length > 0) {
              actions.push({ action: 'setDefaultShippingAddress' as const, addressId: shippingSelected[0] });
            }
            ...
            await updateCustomerAdd({ version: current.version, actions });
...
const handleShippingChange = (id: string) => {
  ...
  setFormData({
    ...
    return (
        ...
        <button type="button" onClick={handleAddAddress}>➕ Add adress</button>
