...
export default function ProfilePageCustomer() {
  ...
  const {id, version, firstName, lastName, dateOfBirth, shippingAddressIds, billingAddressIds, addresses } = customer;
  ...
  return (
    <div>
      <div className="client-block">
        ...
        <button type='button'onClick={() => setShowPasswordModal(true)}>Edit password</button>
      </div>
      <div className="adress-block">
      {enrichedAddresses.map(address => (
        <div key={address.id} style={{ border: '1px solid #ccc', marginBottom: '10px', padding: '10px' }}>
          <p>
            <strong>Адрес:</strong> {address.streetName}, {address.city}, {address.postalCode}, {address.country}
          </p>
          <p>
            {address.isShipping && <span style={{ color: 'green', marginRight: 10 }}>Shipping</span>}
            {address.isBilling && <span style={{ color: 'blue' }}> Billing</span>}
            {!address.isShipping && !address.isBilling && <span style={{ color: 'gray' }}>Без меток</span>}
          </p>
        </div>
      ))}
