import { Link, useLoaderData, Outlet, useMatches } from 'react-router-dom';
...
const CatalogPageTest = () => {
  ...
  const isProductPage = matches.some((match) => match.pathname.includes('/catalog/product/'));
    ...
    <>
      ...
      <ul>
        {products.map((product: Product) => {
          ...
          return (
            <li key={id}>
              <Link to={`/catalog/product/${name.en.toLowerCase()}`} state={{ id }}>
